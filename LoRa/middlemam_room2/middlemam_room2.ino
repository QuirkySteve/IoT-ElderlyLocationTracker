...
void forwardMessageToA(uint8_t senderID, uint8_t* originalMsg, uint8_t msgLength, int16_t rssi) {
   ...
   memcpy(&newMsg[4], originalMsg + 4, msgLength);  // ✅ Copy message payload
   ...
   // ✅ Debugging print
   Serial.print("Forwarding Packet to A from ");
   Serial.print((char)DEVICE_ID);
   Serial.print(": ");
   for (int i = 0; i < (7 + msgLength); i++) {
       Serial.print(newMsg[i], HEX);
       Serial.print(" ");
   }
   Serial.println();
